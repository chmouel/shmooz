...
use crate::{
    ...
    state::{
        ...
        KeyboardTextState, REPEAT_INTERVAL, TextAnnotation,
    },
    window,
    zoom::{
        Point, Size, aspect_ratio, fit_view_to_rect, screen_center, screen_to_source,
        source_to_screen, zoom_towards_factor,
    },
};
...
pub fn repeat_timer_tick(state: &mut AppState) {
    ...
    state.repeat_deadline = Some(Instant::now() + REPEAT_INTERVAL);
}
...
fn pointer_motion(state: &mut AppState, x: f64, y: f64) {
    ...
    let delta_x = x - window.pointer_x;
    let delta_y = y - window.pointer_y;
    let pointer_pressed = window.pointer_pressed;
    let is_shift_selecting = window.shift_select_start.is_some();
    window.pointer_x = x;
    window.pointer_y = y;
    ...
    if state.interaction_mode.is_annotating() {
        if !pointer_pressed {
            overlay::refresh_annotation_overlay(state, output_id);
        } else if state.effective_annotation_tool() == AnnotationTool::Move {
...
fn pointer_button(
    ...
    match button {
        BTN_LEFT if button_state == wl_pointer::ButtonState::Pressed => {
            begin_navigate_press(state, output_id, time);
        }
        BTN_LEFT => end_navigate_press(state, output_id),
        BTN_RIGHT if button_state == wl_pointer::ButtonState::Released => {
            state.request_exit();
        }
        _ => {}
...
/// Starts a pan, a shift-drag box zoom, or resets the view on double click.
fn begin_navigate_press(state: &mut AppState, output_id: u32, time: u32) {
    ...
    let shift_select_start = is_shift
        .then(|| screen_to_annotation_point(state, output_id, window.pointer_x, window.pointer_y));

    let Some(window) = state.windows.get_mut(&output_id) else {
        return;
    };
    if is_shift {
        window.shift_select_start = shift_select_start;
...
/// Ends a pan, or zooms to the selected box when a shift-drag moved far enough.
fn end_navigate_press(state: &mut AppState, output_id: u32) {
    let Some(window) = state.windows.get_mut(&output_id) else {
        return;
    };
    window.pointer_pressed = false;
    let Some(start) = window.shift_select_start.take() else {
        return;
    };

    let (pointer_x, pointer_y) = (window.pointer_x, window.pointer_y);
    let end = screen_to_annotation_point(state, output_id, pointer_x, pointer_y);
    let start_screen = annotation_to_screen_point(state, output_id, start);
    if (pointer_x - start_screen.x).abs() >= 5.0 || (pointer_y - start_screen.y).abs() >= 5.0 {
        let buffer_size = buffer_size(state, output_id);
        let ratio = aspect_ratio(logical_size(state, output_id), buffer_size);
        let target = fit_view_to_rect(start.into(), end.into(), ratio, buffer_size);
...
fn copy_color_to_clipboard(state: &mut AppState, serial: u32) {
    if let Some(output_id) = active_window_id(state) {
        ...
        match screenshot::output_color_at_pointer(state, output_id)
            .and_then(|color| clipboard::set_text_selection(state, serial, color))
        {
            Ok(()) => {
...
fn restore_all_windows(state: &mut AppState) {
    for output_id in state.window_ids() {
        window::cancel_zoom_animation(state, output_id);
        if let Some(window) = state.windows.get_mut(&output_id) {
            window.view_source = window.initial_view_source;
        }
        window::render_window(state, output_id);
    }
}
...
fn palette_index_for_key(key: u32) -> Option<usize> {
    // KEY_1..KEY_0, KEY_MINUS and KEY_EQUAL are consecutive evdev codes.
    (KEY_1..=KEY_EQUAL)
        .contains(&key)
        .then(|| (key - KEY_1) as usize)
}
...
fn annotation_to_screen_point(state: &AppState, output_id: u32, point: AnnotationPoint) -> Point {
    ...
    match state.windows.get(&output_id) {
        Some(window) => source_to_screen(point, window.view_source, logical_size(state, output_id)),
        None => point,
    }
}
...
mod tests {
    ...
    use crate::{
        config::{CloseKey, Config, test_config},
    ...
    fn palette_updates_active_annotation_preview_color() {
        ...
        assert_eq!(stroke.color, ANNOTATION_COLOR_PALETTE[3]);
    }
